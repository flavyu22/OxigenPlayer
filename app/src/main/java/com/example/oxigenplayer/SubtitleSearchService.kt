...
class SubtitleSearchService {
    ...
    suspend fun searchSubtitles(query: String, lang: String = "ro,en", token: String? = null): SearchResult {
        return withContext(Dispatchers.IO) {
            ...
            try {
                val openSubResult = searchOpenSubtitles(query, lang, token)
                results.addAll(openSubResult.subtitles)
                if (openSubResult.errorMessage != null) errors.add("OpenSub: ${openSubResult.errorMessage}")
            } catch (e: Exception) {
                errors.add("OpenSub error.")
            }

            // 2. Search SubsRo (Simulated/Generic Romanian Provider)
            try {
                val subsRoResult = searchSubsRo(query, lang)
                results.addAll(subsRoResult.subtitles)
            } catch (e: Exception) {}

            // 3. Relevance Filtering
            val filteredResults = filterRelevantResults(results, query)

            if (filteredResults.isEmpty() && errors.isNotEmpty()) {
                SearchResult(emptyList(), errors.joinToString(" "))
            } else {
                SearchResult(filteredResults.sortedByDescending { it.language == "RO" }, null)
            }
        }
    }

    private fun searchOpenSubtitles(query: String, lang: String, token: String?): SearchResult {
        val cleanQuery = cleanQuery(query)
        if (cleanQuery.isEmpty()) return SearchResult(emptyList(), "Nume invalid.")
        ...
        if (conn.responseCode == 200) {
            ...
            return SearchResult(list, null)
        } else {
            return SearchResult(emptyList(), "OS Err ${conn.responseCode}")
        }
    }

    private fun searchSubsRo(query: String, lang: String): SearchResult {
        // Subs.ro logic placeholder - implementation would normally require a scraper
        // For now, we ensure results from OpenSubtitles are prioritized and filtered
        return SearchResult(emptyList(), null)
    }

    private fun filterRelevantResults(results: List<SubtitleSearchResult>, query: String): List<SubtitleSearchResult> {
        val rawCleanQuery = query.lowercase().replace(Regex("[^a-z0-9 ]"), " ").trim()
        val queryWords = rawCleanQuery.split(" ").filter { it.length > 2 && it != "the" && it != "and" }
        
        // Extract year from query if exists (e.g. "2023")
        val yearInQuery = Regex("\\b(19|20)\\d{2}\\b").find(query)?.value

        if (queryWords.isEmpty()) return results

        return results.filter { res ->
            val movieTitle = res.movieName.lowercase()
            val fileName = res.fileName.lowercase()
            
            // 1. Mandatory Year match if year exists in both
            if (yearInQuery != null && res.year.isNotEmpty() && res.year != yearInQuery) {
                return@filter false
            }

            // 2. Strict word matching: At least 70% of query words must match
            // We use 70% to allow for small variations in title
            val matchCount = queryWords.count { word -> movieTitle.contains(word) || fileName.contains(word) }
            val matchRatio = if (queryWords.isNotEmpty()) matchCount.toFloat() / queryWords.size else 0f
            
            matchRatio >= 0.7f || movieTitle.contains(rawCleanQuery) || rawCleanQuery.contains(movieTitle)
        }
    }
